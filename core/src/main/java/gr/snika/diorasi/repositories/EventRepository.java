...
public interface EventRepository extends CrudRepository<Event, UUID> {
	...
	@Query(value="SELECT e.URL AS filterfield, count(*) AS numberofevents FROM event e WHERE e.website_id=:websiteId AND e.event_type = 'pageview' AND e.created_at BETWEEN :dateFrom AND :dateTo GROUP BY 1 ORDER BY numberofevents DESC", nativeQuery=true)
	List<EventsCountDTO> findPagesCount(@Param("websiteId") UUID websiteId, @Param("dateFrom") LocalDate dateFrom, @Param("dateTo") LocalDate dateTo);
}
